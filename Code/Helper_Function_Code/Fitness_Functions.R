...
###Output: The maximum cumulative transmission potential                #
#########################################################################  
Gametocyte_Fitness_LXMX <- function(x){
  ...
  end_fitness <- max(cumsum(exp(-(1/100)* gam_dat$time)*(PrI_PC(gam_dat$gam) * 1/10)))
...
Finder_RM_lxmx <- function(x_list, mu_M_c){
  ...
  if(mort_time$surv == "surv"){
    ...
    end_fitness_mort <-  max(cumsum(exp(-(1/100)* truncate_G_TS$time)*(PrI_PC(truncate_G_TS$G) * 1/10)))
