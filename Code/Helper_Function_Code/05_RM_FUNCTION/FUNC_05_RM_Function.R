...
rate_PMR <- function(x, B_V, C_V, mu_M){
  p =   4.0e-6
  rate = 
 rate = ((100* (1))/(100 + 0.025))^100 *((1 - C_V) * B_V)*((x[,"R"] * p)/((p*x[,"R"]) + mu_M))
...
rate_PMR_data <- function(x, y, B_Vec, C_Value, mu_M){
  ...
  Data_Interest_Split <- split(Data_Interest, 
                               Data_Interest$B_V)
  ...
  for (k in seq(1, length(Data_Interest_Split ))){
    ...
    #Gametocytes 
    Gam_Time <- Data_Interest_Split[[k]][,"G"]
    ...
    ###The approxfuns 
    Gam_Func <- approxfun(Data_Interest_Split[[k]][,"time"],
                          Gam_Time)
    ...
    Gam_End = Gam_Func(End_Interest[k,]$endtime)
    ...
    rate_PMR_DF[[k]] <- cbind.data.frame(
      Gam_Time,PMR_B,
      Daily_Trans_Prob, 
